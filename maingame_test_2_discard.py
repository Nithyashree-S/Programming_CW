...
discard_button_width, discard_button_height = 120, 40
discard_button_x = 615
discard_button_y = 200
discard_button_area = pygame.Rect(discard_button_x, discard_button_y, discard_button_width, discard_button_height)
...
yes_button_width, yes_button_height = 80, 40
yes_button_x, yes_button_y = 300, 400
yes_button_area = pygame.Rect(yes_button_x, yes_button_y, yes_button_width, yes_button_height)

no_button_width, no_button_height = 80, 40
no_button_x, no_button_y = 400, 400
no_button_area = pygame.Rect(no_button_x, no_button_y, no_button_width, no_button_height)
...
def display_cards(player_id, x, y, spacing=30):
    ...
    if valid_groups[player_id]:
        ...
        label_rect = label.get_rect(center=(current_x - 20, y - 5))
...
def check_hand_validity(player_id):
    ...
    # Remove extra newline before first check
    print(f"\nChecking {player_name}'s hand for valid groups...")
    ...
    # Remove extra newline before second check
    ...
    if largest_group and len(largest_group) > 3:
        cards_str = ", ".join(str(card) for card in largest_group)
        message = f"{player_name} has a larger group: {cards_str}"
        print(f"Found color run of length {len(largest_group)}: {[str(card) for card in largest_group]}")
    else:
        print("No largest valid group found")
    
    # Only add a single newline between player checks
    if player_id == 1:
...
def ai_turn():
    ...
    # Random decision between draw, snatch, or skip
    ...
    if action == 'draw':
        if is_hand_full(2):
            ...
            # Second phase: Animate cards moving to Computer's hand
            ...
            handle_card_addition(2)  # Only check hand when cards are added
            pygame.time.wait(500)
        ...
        if is_hand_full(2):
            ...
            handle_card_addition(2)  # Check Computer's hand after adding card
            check_hand_validity(1)   # Check Human's hand after card removal
        else:
...
def discard_group(player_id):
    """Discard the valid group or the largest valid group from the player's hand and reshuffle into the deck."""
    global valid_groups, largest_groups, full_deck, message
    
    if largest_groups[player_id]:
        # Discard the largest group
        group_to_discard = largest_groups[player_id]
        message = "Discarded largest group."
    elif valid_groups[player_id]:
        # Discard the valid group
        group_to_discard = valid_groups[player_id]
        message = "Discarded valid group."
    else:
        message = "No valid group to discard!"
        return
    
    # Remove the group cards from the player's hand
    player_hands[player_id] = [card for card in player_hands[player_id] if card not in group_to_discard]
    valid_groups[player_id] = None
    largest_groups[player_id] = None

    # Add discarded cards back into the deck
    full_deck.extend([str(card) for card in group_to_discard])
    random.shuffle(full_deck)  # Reshuffle the deck

    print(f"Player {player_id} discarded: {[str(card) for card in group_to_discard]}")
...
while running:
            ...
            if current_player == 1 and not dealing:
                ...
                # Done Drawing button
                    ...
                    if len(drawn_cards) == 0:
                        print("Human skips turn")  # Just print skip message
                        current_player = 2  # Change to computer's turn
                    else:
                        if can_add_cards(current_player, len(drawn_cards)):
                            player_hands[current_player].extend(drawn_cards)
                            handle_card_addition(current_player)  # Only check when cards are added
                            drawn_cards.clear()
                            clear_drawn_card_area()
                            current_player = 2
                        else:
                ...
                elif discard_button_area.collidepoint(mouse_pos):
                   if current_player == 1 and (valid_groups[1] or largest_groups[1]):
                      
                      discard_confirmation = True  # Enable confirmation mode
                      message = "Are you sure you want to discard? (Yes/No)"
                   elif current_player == 1:
                     message = "No valid group to discard!"
                elif discard_confirmation:
                    if yes_button_area.collidepoint(mouse_pos):
                      # Discard group logic
                      discard_group(1)
                      discard_confirmation = False  # Exit confirmation mode
                      current_player = 2  # Pass the turn to AI
    ...
    draw_button("Discard", discard_button_x, discard_button_y, discard_button_width, discard_button_height)
    
    

    if discard_confirmation:
       draw_button("Yes", yes_button_x, yes_button_y, yes_button_width, yes_button_height, color=(0, 200, 0))
       draw_button("No", no_button_x, no_button_y, no_button_width, no_button_height, color=(200, 0, 0))
