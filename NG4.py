...
class GameScreen(Screen):
    ...
    def update(self):
        ...
        if self.draw_button.is_clicked() and self.player_turn == 1:
            ...
            if isinstance(result, Card):
                print(f"Card drawn to table by Player 1: {result}")
            else:
                print(result)  # Print messages like "Max cards drawn" or "Deck is empty"

        # Add Cards from Table to Player 1's Hand
    ...
    def ai_turn(self):
        if self.player_turn == 2:  # AI Turn
            num_cards_to_draw = random.choice([1, 2, 3])
            print(f"AI decides to draw {num_cards_to_draw} card(s).")
            for _ in range(num_cards_to_draw):
                if len(self.game.deck) > 0:
                    card = self.game.deck.pop()
                    self.game.player2_hand.append(card)
                    print(f"AI drew: {card}")
                else:
                    print("Deck is empty. AI cannot draw more cards.")
                    break
            print(f"Player 2 hand: {[str(card) for card in self.game.player2_hand]}")
