...
class Game:
    def __init__(self):
        ...
        self.last_draw_time = None  # Track the last draw time
    ...
    def draw_card(self):
        if self.cards_drawn < 3 and len(self.deck) > 0:  # Allow only up to 3 cards to be drawn
            ...
            self.cards_drawn += 1
            self.last_draw_time = time.time()  # Update the time when the last card was drawn
            return card
    ...
    def update(self):
        ...
        if self.draw_button.is_clicked():
            ...
            if isinstance(result, Card):
                ...
                print(result)
        # Automatically add cards to hand after 5 seconds of inactivity
